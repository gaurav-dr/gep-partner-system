  ...
  useEffect(() => {
    async function testConnection() {
      ...
      try {
        ...
        try {
          const partnersPromise = supabase.from('partners').select('*').limit(5);
    ...
    <div className="px-4 sm:px-0">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {connectionTest ? (
            <div
              className={`p-4 rounded-md ${connectionTest.status === 'success' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}
            >
              <p>
                <strong>Status:</strong> {connectionTest.status}
              </p>
              <p>
                <strong>Message:</strong> {connectionTest.message}
              </p>
              <p>
                <strong>Time:</strong> {connectionTest.timestamp}
              </p>
            </div>
          ) : (
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {partnersData ? (
            <div>
              <p className="mb-2">
                <strong>Count:</strong> {partnersData.count || 0} records
              </p>
              <p className="mb-2">
                <strong>Error:</strong>{' '}
                {partnersData.error ? JSON.stringify(partnersData.error) : 'None'}
              </p>
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {requestsData ? (
            <div>
              <p className="mb-2">
                <strong>Count:</strong> {requestsData.count || 0} records
              </p>
              <p className="mb-2">
                <strong>Error:</strong>{' '}
                {requestsData.error ? JSON.stringify(requestsData.error) : 'None'}
              </p>
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {assignmentsData ? (
            <div>
              <p className="mb-2">
                <strong>Count:</strong> {assignmentsData.count || 0} records
              </p>
              <p className="mb-2">
                <strong>Error:</strong>{' '}
                {assignmentsData.error ? JSON.stringify(assignmentsData.error) : 'None'}
              </p>
