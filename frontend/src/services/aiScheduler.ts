...
class AIScheduler {
  ...
  private async evaluatePartnerForRequest(
    partner: Partner,
    ...
    // Calculate totals
    const totalEstimatedHours = installationAssignments.reduce(
      (sum, ia) => sum + ia.recommended_visits * ia.estimated_hours_per_visit,
      0
    );
  ...
  private calculateInstallationAssignments(request: CustomerRequest): InstallationAssignment[] {
    ...
    const installationsPerRequest = Math.min(
      request.number_of_installations,
      ...
      for (let visitNum = 0; visitNum < assignment.recommended_visits; visitNum++) {
        ...
        const visit: ScheduledVisit = {
          ...
          visit_type:
            visitNum === 0
              ? 'initial'
              : visitNum === assignment.recommended_visits - 1
                ? 'final'
                : 'follow_up',
        };
  ...
  private isDateBlocked(date: Date, partner: Partner, request: CustomerRequest): boolean {
    ...
    const hasConflict = this.existingSchedules.some(
      visit =>
  ...
  private calculateLocationProximity(partner: Partner, request: CustomerRequest): number {
    ...
    const sameLocation =
      request.location.toLowerCase().includes(partner.city.toLowerCase()) ||
  ...
  private generateReasoning(partner: Partner, request: CustomerRequest, scores: any): string[] {
    ...
    if (scores.specialtyMatchScore > 90) {
      reasoning.push(
        `Perfect specialty match: ${partner.specialty} ideal for ${request.work_type}`
      );
    }

    if (scores.availabilityScore > 85) {
      reasoning.push(
        `High availability: ${partner.experience_years} years experience, ${partner.rating}/5.0 rating`
      );
    }
