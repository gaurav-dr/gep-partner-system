...
test.describe('Dashboard Data Validation', () => {
  ...
  test('should display real database stats instead of hardcoded values', async ({ page }) => {
    ...
    const totalRequests = await page.locator('p:has-text("Total Requests") + * p').first().textContent();
    const activePartners = await page.locator('p:has-text("Active Partners") + * p').first().textContent();
    const pendingAssignments = await page.locator('p:has-text("Pending Assignments") + * p').first().textContent();
    const completedThisMonth = await page.locator('p:has-text("Completed This Month") + * p').first().textContent();
