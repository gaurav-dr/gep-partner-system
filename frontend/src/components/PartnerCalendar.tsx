...
const PartnerCalendar: React.FC<PartnerCalendarProps> = ({ partner }) => {
    ...
    const generateAssignments = () => {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        sampleAssignments.push({
          id: `assign-past-${i}`,
          customer_name: ['ACME Corp', 'TechnoSoft SA', 'MediCare Ltd', 'BuildCorp'][
            Math.floor(Math.random() * 4)
          ],
          installation_address: ['ΛΕΩΦ. ΣΥΓΓΡΟΥ 350', 'ΜΙΧΑΛΑΚΟΠΟΥΛΟΥ 98', 'ΒΑΣΙΛΙΣΣΗΣ ΣΟΦΙΑΣ 125'][
      ...
      for (let i = 0; i < 6; i++) {
        ...
        sampleAssignments.push({
          ...
          installation_address: ['ΚΗΦΙΣΙΑΣ 230', 'ΠΑΤΗΣΙΩΝ 145', 'ΑΛΕΞΑΝΔΡΑΣ 87'][
            Math.floor(Math.random() * 3)
          ],
          visit_date: date.toISOString().split('T')[0],
  ...
  const pastAssignments = assignments.filter(
    a => new Date(a.visit_date) < new Date() && a.status === 'completed'
  );
  const upcomingAssignments = assignments.filter(
    a => new Date(a.visit_date) >= new Date() && a.status === 'confirmed'
  );
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-md">
        <div className="px-4 py-5 sm:p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            {viewMode === 'month' ? 'This Month' : 'This Week'} ({filteredAssignments.length}{' '}
            visits)
          </h3>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span
                          className={`px-2 py-1 text-xs rounded-full font-medium ${getStatusColor(assignment.status)}`}
                        >
                      ...
                      <div className="space-y-1 text-sm text-gray-600">
                        <div className="flex items-center space-x-4">
                          ...
                          <span>
                            🕐 {formatDate(assignment.visit_date)} at{' '}
                            {formatTime(assignment.visit_time)}
                          </span>
                        </div>
                        <div className="flex items-center space-x-4">
                          ...
                          <span className="capitalize">
                            {assignment.visit_type.replace('_', ' ')} visit
                          </span>
                        </div>
                      </div>
