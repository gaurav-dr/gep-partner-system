    ...
    () => schedulesApi.getByDateRange(start, end),
    {
      select: data => {
        return data.map((visit: any) => ({
          ...
          type:
            visit.schedules?.service_type === 'occupational_doctor' ? 'inspection' : 'consultation',
                ...
                .map((event, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${getEventColor(event.type, event.status)}`}
                      >
                        {event.status}
                      </span>
                    </div>
