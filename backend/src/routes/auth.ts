...
import { Logger, AuthRequest, RegistrationRequest, LoginRequest } from '../types';
 ...
 * @access  Public
 */
router.post('/login', authLimiter, validateLogin, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { email, password }: LoginRequest = req.body;
 ...
 * @access  Public
 */
router.post('/request-password-reset', passwordResetLimiter, async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.get('/users', authenticate, authorize('admin'), async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        res.json({
            success: true,
            users: users.map((user: any) => AuthService.sanitizeUser(user))
        });
