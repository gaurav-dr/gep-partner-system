...
const updatePartnerSchema = createPartnerSchema.fork(['id'], (schema: any) => schema.optional());
...
router.put('/:id', validateRequest(updatePartnerSchema), async (req: Request<{ id: string }, Partner, UpdatePartnerRequest>, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
